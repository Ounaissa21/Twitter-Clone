# classify_api.py
import json
import numpy as np
import tensorflow as tf
from fastapi import FastAPI, BackgroundTasks, HTTPException
from pydantic import BaseModel
from tensorflow.keras.preprocessing.sequence import pad_sequences
from tensorflow.keras.preprocessing.text import tokenizer_from_json
from nltk.tokenize import word_tokenize
from nltk.corpus import stopwords
from nltk.stem import WordNetLemmatizer
from textblob import TextBlob
import re
import string
import nltk
import os
import httpx # Import httpx for async HTTP requests
import asyncio # For simulating async operations
import logging # Import the logging module

# --- Logging Configuration ---
# Configure basic logging to output to console
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

# --- Appwrite Configuration (Update with your local details) ---
APPWRITE_ENDPOINT = "http://192.168.137.1/v1" # Or your Appwrite's exposed IP/Port
APPWRITE_PROJECT_ID = "682e3f8f000d76bff0a2" # Get this from your Appwrite console
APPWRITE_API_KEY = "standard_"   # Create an API Key in Appwrite with 'databases.write' permission
APPWRITE_DATABASE_ID = "682e435d003a1afad98c"     # Your database ID
APPWRITE_TWEETS_COLLECTION_ID = "682e43ee00001dd447c2" # Your tweets collection ID

# --- NLTK Data Downloads (remains the same but with logging) ---
nltk_data_path = os.environ.get("NLTK_DATA")
if nltk_data_path:
    nltk.data.path.insert(0, nltk_data_path) # Add custom path to NLTK search paths
    logger.info(f"Added {nltk_data_path} to NLTK data search path.")
else:
    nltk_data_path = nltk.data.path[0]
    logger.info(f"NLTK_DATA environment variable not set, using default download path: {nltk_data_path}")
logger.info("Checking for NLTK resources for API script...")
try:
    nltk.data.find('tokenizers/punkt')
    logger.info("NLTK 'punkt' found.")
except LookupError:
    logger.info("NLTK 'punkt' not found. Attempting download during startup...")
    try:
        nltk.download('punkt', download_dir=nltk_data_path, quiet=True) # Pass download_dir
        logger.info("NLTK 'punkt' downloaded.")
    except Exception as e:
        logger.error(f"Error downloading 'punkt' during startup: {e}") # More specific error
try:
    nltk.data.find('corpora/stopwords')
    logger.info("NLTK 'stopwords' found.")
except LookupError:
    logger.info("NLTK 'stopwords' not found. Attempting download during startup...")
    try:
        nltk.download('stopwords', download_dir=nltk_data_path, quiet=True)
        logger.info("NLTK 'stopwords' downloaded.")
    except Exception as e:
        logger.error(f"Error downloading 'stopwords' during startup: {e}")

try:
    nltk.data.find('corpora/wordnet')
    logger.info("NLTK 'wordnet' found.")
except LookupError:
    logger.info("NLTK 'wordnet' not found. Attempting download during startup...")
    try:
        nltk.download('wordnet', download_dir=nltk_data_path, quiet=True)
        logger.info("NLTK 'wordnet' downloaded.")
    except Exception as e:
        logger.error(f"Error downloading 'wordnet' during startup: {e}")

try:
    nltk.data.find('corpora/omw-1.4')
    logger.info("NLTK 'omw-1.4' found.")
except LookupError:
    logger.info("NLTK 'omw-1.4' not found. Attempting download during startup...")
    try:
        nltk.download('omw-1.4', download_dir=nltk_data_path, quiet=True)
        logger.info("NLTK 'omw-1.4' downloaded.")
    except Exception as e:
        logger.error(f"Error downloading 'omw-1.4' during startup: {e}")

# Download the vader_lexicon for TextBlob's sentiment analysis
try:
    nltk.data.find('sentiment/vader_lexicon')
    logger.info("NLTK 'vader_lexicon' found.")
except LookupError:
    logger.info("NLTK 'vader_lexicon' not found. Attempting download during startup...")
    try:
        nltk.download('vader_lexicon', download_dir=nltk_data_path, quiet=True)
        logger.info("NLTK 'vader_lexicon' downloaded.")
    except Exception as e:
        logger.error(f"Error downloading 'vader_lexicon' during startup: {e}")

logger.info("NLTK resource check complete for API script.")

stop_words = set(stopwords.words('english'))
sentiment_words_to_keep = {'love', 'hate', 'like', 'dislike', 'good', 'bad', 'not', 'very', 'too', 'less', 'more', 'no',
                           'healthy', 'unhealthy', 'sick', 'well', 'pain', 'food', 'eat', 'drink', 'exercise', 'stress',
                           'never', 'none', 'nothing', 'nowhere', 'hardly', 'scarcely', 'barely', 'aint', 'isnt', 'arent',
                           'wasnt', 'werent', 'hasnt', 'havent', 'hadnt', 'wont', 'wouldnt', 'dont', 'doesnt', 'didnt',
                           'cant', 'couldnt', 'shouldnt', 'mightnt', 'mustnt'}
stop_words = stop_words - sentiment_words_to_keep

# --- Lemmatizer Initialization ---
lemmatizer = WordNetLemmatizer()

OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY")

if not OPENROUTER_API_KEY:
    logger.error("Error: OPENROUTER_API_KEY environment variable not set!")
OPENROUTER_URL = "https://openrouter.ai/api/v1/chat/completions"

def preprocess_text(text):
    """
    Cleans and preprocesses text: lowercases, removes URLs, mentions, hashtags, punctuation,
    tokenizes, handles simple negation, removes custom stopwords, and lemmatizes.
    This function MUST be identical to the one used during training.
    Handles potential non-string inputs gracefully.
    """
    if not isinstance(text, str):
        return ""
    text = text.lower()
    text = re.sub(r'http\S+|www\S+|https\S+', '', text, flags=re.MULTILINE)
    text = re.sub(r'@\w+', '', text)
    text = re.sub(r'#', '', text)
    text = re.sub(f'[{re.escape(string.punctuation)}]', '', text)
    word_tokens = word_tokenize(text)

    negation_words = {'not', 'no', 'never', 'none', 'nothing', 'nowhere', 'hardly', 'scarcely', 'barely',
                      'aint', 'isnt', 'arent', 'wasnt', 'werent', 'hasnt', 'havent', 'hadnt', 'wont',
                      'wouldnt', 'dont', 'doesnt', 'didnt', 'cant', 'couldnt', 'shouldnt', 'mightnt', 'mustnt'}
    processed_tokens = []
    i = 0
    while i < len(word_tokens):
        word = word_tokens[i]
        if word in negation_words and i + 1 < len(word_tokens):
            processed_tokens.append(word + '_' + word_tokens[i+1])
            i += 2
        else:
            processed_tokens.append(word)
            i += 1

    filtered_words = [
        lemmatizer.lemmatize(word) for word in processed_tokens if word not in stop_words and word.replace('_', '').isalpha()
    ]
    return " ".join(filtered_words)

def analyze_sentiment(text):
    """Analyzes sentiment using TextBlob."""
    if not isinstance(text, str) or not text.strip():
        return 0.0
    try:
        return TextBlob(text).sentiment.polarity
    except Exception as e:
        logger.error(f"Error during sentiment analysis: {e}")
        return 0.0

unhealthy_keywords = [
    "pizza", "burger", "fries", "soda", "sugar", "candy", "chocolate",
    "smoking", "alcohol", "junk food", "fast food", "sedentary", "couch potato",
    "stay up late", "all night", "excessive screen time", "unhealthy food", "cigarettes",
    "binge eating", "crash diet", "skipping meals", "too much screen time", "lack of sleep"
]
strong_unhealthy_keywords = [
    "smoking", "alcohol", "cigarettes", "binge eating", "crash diet", "skipping meals",
    "excessive screen time", "lack of sleep", "stay up all night"
]
avoidance_keywords = [
    "quit", "stopped", "avoid", "hate", "dislike", "giving up", "staying away from",
    "don't eat", "don't drink", "not eating", "not drinking", "avoiding"
]
warning_keywords = [
    "bad for your health", "harmful", "dangerous", "risk", "causes", "leads to",
    "beware of", "warning", "should avoid", "negative effects", "detrimental"
]

known_healthy_habits = [
    "working out in the morning", "exercise", "eating vegetables",
    "drinking water", "sleeping well", "meditation", "yoga", "healthy food",
    "running", "swimming", "gym", "workout", "salad", "fruit", "vegetables",
    "balanced diet", "staying hydrated", "getting enough sleep", "mindfulness", "walking",
    "clean eating", "portion control"
]

def contains_keyword(text, keywords):
    """Checks if the text contains any of the specified keywords."""
    if not isinstance(text, str):
        return False
    text_lower = text.lower()
    for keyword in keywords:
        if re.search(r'\b' + re.escape(keyword) + r'\b', text_lower):
            return True
    return False

# This function is now async because it might be called in a background task
async def generate_persuasive_message(unhealthy_habit_text):
    """Generates a persuasive message to encourage a healthier choice."""
    if not OPENROUTER_API_KEY:
        logger.warning("Skipping persuasive message generation: OPENROUTER_API_KEY not set.")
        return "Could not generate a persuasive message (API key not set)."

    logger.info("\n--- Generating Persuasive Message ---")
    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json"
    }
    prompt = f"Reply to this tweet persuasively, encouraging a healthier choice: '{unhealthy_habit_text}'"
    data = {
        "model": "mistralai/mixtral-8x7b-instruct",
        "messages": [
            {"role": "system", "content": "You are a helpful and encouraging health assistant. Respond like a tweet reply—concise, persuasive, and engaging. Focus on motivating healthier choices."},
            {"role": "user", "content": prompt}
        ],
        "max_tokens": 75,
        "temperature": 0.8
    }
    message = "Could not generate a persuasive message." # Default message if API call fails

    try:
        # Use httpx.AsyncClient for asynchronous requests
        async with httpx.AsyncClient() as client:
            response = await client.post(OPENROUTER_URL, json=data, headers=headers, timeout=30.0) # Added timeout
        response.raise_for_status()
        response_json = response.json()
        message = response_json.get("choices", [{}])[0].get("message", {}).get("content", message)
        logger.info("🗣️ Persuasive Message Generated.")
    except httpx.RequestError as e:
        logger.error(f"❌ HTTPX Request Error generating persuasive message: {e}")
        message = f"Error generating message (network/request issue): {e}"[:100] + "..."
    except httpx.HTTPStatusError as e:
        logger.error(f"❌ HTTPX Status Error generating persuasive message: {e.response.status_code} - {e.response.text}")
        message = f"Error generating message (API returned error): {e.response.status_code}"[:100] + "..."
    except Exception as e:
        logger.error(f"❌ An unexpected error occurred during message generation: {e}")
        message = f"Unexpected error generating message: {e}"[:100] + "..."
    return message

# --- Model Loading (remains the same but with logging) ---
MODEL_PATH = "best_cnn_lstm_model_deeper.keras"
TOKENIZER_PATH = "tokenizer_config.json"
MAX_LEN = 100
logger.info(f"Loading model from {MODEL_PATH}...")
if not os.path.exists(MODEL_PATH):
    logger.error(f"Error: Model file not found at {MODEL_PATH}")
    logger.error("Please ensure the model file exists in the same directory.")
    exit()
try:
    model = tf.keras.models.load_model(MODEL_PATH)
    logger.info("Model loaded successfully.")
except Exception as e:
    logger.error(f"Error loading model: {e}")
    exit()

logger.info(f"Loading tokenizer from {TOKENIZER_PATH}...")
if not os.path.exists(TOKENIZER_PATH):
    logger.error(f"Error: Tokenizer file not found at {TOKENIZER_PATH}")
    logger.error("Please ensure 'tokenizer_config.json' exists in the same directory.")
    exit()
try:
    with open(TOKENIZER_PATH, 'r', encoding='utf-8') as f:
        tokenizer_json_string = f.read()
    tokenizer = tokenizer_from_json(tokenizer_json_string)
    logger.info("Tokenizer loaded successfully.")
except Exception as e:
    logger.error(f"Error loading tokenizer: {e}")
    exit()

model_high_confidence_threshold = 0.8
negative_sentiment_threshold = -0.1
positive_sentiment_threshold = 0.1
model_default_threshold = 0.5
strong_override_sentiment_threshold = 0.5
negative_sentiment_unhealthy_avoidance_override_threshold = -0.3

app = FastAPI()

# --- Shared Classification Logic (extracted for reuse) ---
async def _classify_text_logic(original_text: str): # Made async because generate_persuasive_message is async
    """
    Contains the core classification logic, including preprocessing, model prediction,
    sentiment analysis, keyword matching, and rule-based post-processing.
    """
    logger.info(f"\n--- Starting Classification Logic for: '{original_text}' ---")

    if not original_text or not original_text.strip():
        logger.warning("Input text is empty.")
        return {"category": "Could not classify", "confidence": 0.0, "detail": "Input text is empty."}

    processed_text = preprocess_text(original_text)
    logger.info(f"Processed Text for Model: '{processed_text}'")

    if not processed_text.strip():
        logger.warning("Warning: Preprocessing resulted in empty text. Attempting keyword fallback.")
        sentiment_polarity = analyze_sentiment(original_text)
        contains_any_unhealthy_kwd = contains_keyword(original_text, unhealthy_keywords)
        contains_strong_unhealthy_kwd = contains_keyword(original_text, strong_unhealthy_keywords)
        contains_any_healthy_kwd = contains_keyword(original_text, known_healthy_habits)
        contains_avoidance_kwd = contains_keyword(original_text, avoidance_keywords)
        contains_warning_kwd = contains_keyword(original_text, warning_keywords)

        final_classification = "Could not classify (Empty after preprocessing)"
        detail = "Preprocessing resulted in empty text. Attempted keyword fallback."
        persuasive_message = "No message generated due to preprocessing issue."
        model_confidence = 0.0

        if contains_strong_unhealthy_kwd:
            final_classification = "unhealthy"
            detail = "Classified as UNHEALTHY based on strong keyword in original text (empty after preprocessing)."
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        elif contains_any_unhealthy_kwd and not contains_any_healthy_kwd:
            final_classification = "unhealthy"
            detail = "Classified as UNHEALTHY based on any unhealthy keyword in original text (empty after preprocessing)."
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        elif contains_any_healthy_kwd and not contains_any_unhealthy_kwd:
            final_classification = "healthy"
            detail = "Classified as HEALTHY based on healthy keyword in original text (empty after preprocessing)."
            persuasive_message = "No message generated (classified as healthy)."
        else:
            final_classification = "Could not confidently classify"
            detail = "Preprocessing resulted in empty text. Keyword fallback inconclusive."
            persuasive_message = "No message generated (could not classify)."
        logger.info(f"Fallback Classification: {final_classification}, Detail: {detail}")
        return {
            "category": final_classification.capitalize(),
            "confidence": model_confidence,
            "detail": detail,
            "persuasive_message": persuasive_message
        }

    sequences = tokenizer.texts_to_sequences([processed_text])
    if not sequences or not sequences[0]:
        logger.warning("Warning: Tokenizer returned empty sequence after preprocessing. Attempting keyword fallback.")
        sentiment_polarity = analyze_sentiment(original_text)
        contains_any_unhealthy_kwd = contains_keyword(original_text, unhealthy_keywords)
        contains_strong_unhealthy_kwd = contains_keyword(original_text, strong_unhealthy_keywords)
        contains_any_healthy_kwd = contains_keyword(original_text, known_healthy_habits)
        contains_avoidance_kwd = contains_keyword(original_text, avoidance_keywords)
        contains_warning_kwd = contains_keyword(original_text, warning_keywords)

        final_classification = "Could not classify (Empty after tokenization)"
        detail = "Tokenizer returned empty sequence. Attempted keyword fallback."
        persuasive_message = "No message generated due to tokenization issue."
        model_confidence = 0.0

        if contains_strong_unhealthy_kwd:
            final_classification = "unhealthy"
            detail = "Classified as UNHEALTHY based on strong keyword in original text (empty after tokenization)."
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        elif contains_any_unhealthy_kwd and not contains_any_healthy_kwd:
            final_classification = "unhealthy"
            detail = "Classified as UNHEALTHY based on any unhealthy keyword in original text (empty after tokenization)."
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        elif contains_any_healthy_kwd and not contains_any_unhealthy_kwd:
            final_classification = "healthy"
            detail = "Classified as HEALTHY based on healthy keyword in original text (empty after tokenization)."
            persuasive_message = "No message generated (classified as healthy)."
        else:
            final_classification = "Could not confidently classify"
            detail = "Tokenizer returned empty sequence. Keyword fallback inconclusive."
            persuasive_message = "No message generated (could not classify)."

        logger.info(f"Fallback Classification: {final_classification}, Detail: {detail}")
        return {
            "category": final_classification.capitalize(),
            "confidence": model_confidence,
            "detail": detail,
            "persuasive_message": persuasive_message
        }

    padded = pad_sequences(sequences, maxlen=MAX_LEN, padding='post', truncating='post', dtype='float32')

    try:
        predictions = model.predict(padded, verbose=0)
        probability_unhealthy = predictions[0][0]
        probability_healthy = 1 - probability_unhealthy
        model_confidence = float(probability_unhealthy)

        logger.info(f"\n--- Model Prediction (Raw) ---")
        logger.info(f"🔹 Probability (Unhealthy): {probability_unhealthy:.4f}")
        logger.info(f"🔹 Probability (Healthy): {probability_healthy:.4f}")
        logger.info("------------------------------")

    except Exception as e:
        logger.error(f"❌ Error during model prediction: {e}")
        sentiment_polarity = analyze_sentiment(original_text)
        contains_any_unhealthy_kwd = contains_keyword(original_text, unhealthy_keywords)
        contains_strong_unhealthy_kwd = contains_keyword(original_text, strong_unhealthy_keywords)
        contains_any_healthy_kwd = contains_keyword(original_text, known_healthy_habits)
        contains_avoidance_kwd = contains_keyword(original_text, avoidance_keywords)
        contains_warning_kwd = contains_keyword(original_text, warning_keywords)

        final_classification = "Could not classify (Model Error)"
        detail = f"Error during model prediction: {e}"[:100] + "..." + ". Attempted keyword fallback."
        persuasive_message = "No message generated due to model error."
        model_confidence = 0.0

        if contains_strong_unhealthy_kwd:
            final_classification = "unhealthy"
            detail = "Classified as UNHEALTHY based on strong keyword (Model Error Fallback)."
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        elif contains_any_unhealthy_kwd and not contains_any_healthy_kwd:
            final_classification = "unhealthy"
            detail = "Classified as UNHEALTHY based on any unhealthy keyword (Model Error Fallback)."
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        elif contains_any_healthy_kwd and not contains_any_unhealthy_kwd:
            final_classification = "healthy"
            detail = "Classified as HEALTHY based on healthy keyword (Model Error Fallback)."
            persuasive_message = "No message generated (classified as healthy)."
        else:
            final_classification = "Could not confidently classify"
            detail = "Model Error Fallback: Keyword fallback inconclusive."
            persuasive_message = "No message generated (could not classify)."

        logger.info(f"Fallback Classification: {final_classification}, Detail: {detail}")
        return {
            "category": final_classification.capitalize(),
            "confidence": model_confidence,
            "detail": detail,
            "persuasive_message": persuasive_message
        }

    try:
        sentiment_polarity = analyze_sentiment(original_text)
        contains_any_unhealthy_kwd = contains_keyword(original_text, unhealthy_keywords)
        contains_strong_unhealthy_kwd = contains_keyword(original_text, strong_unhealthy_keywords)
        contains_any_healthy_kwd = contains_keyword(original_text, known_healthy_habits)
        contains_avoidance_kwd = contains_keyword(original_text, avoidance_keywords)
        contains_warning_kwd = contains_keyword(original_text, warning_keywords)

        logger.info(f"--- Post-processing Info ---")
        logger.info(f"🔹 Sentiment Polarity: {sentiment_polarity:.4f}")
        logger.info(f"🔹 Contains Any Unhealthy Keyword: {contains_any_unhealthy_kwd}")
        logger.info(f"🔹 Contains Strong Unhealthy Keyword: {contains_strong_unhealthy_kwd}")
        logger.info(f"🔹 Contains Any Healthy Keyword: {contains_any_healthy_kwd}")
        logger.info(f"🔹 Contains Avoidance Keyword: {contains_avoidance_kwd}")
        logger.info(f"🔹 Contains Warning Keyword: {contains_warning_kwd}")
        logger.info("--------------------------")

        logger.info(f"--- Debugging Post-processing Conditions ---")
        logger.info(f"Thresholds: High Conf={model_high_confidence_threshold}, Neg Sent={negative_sentiment_threshold}, Pos Sent={positive_sentiment_threshold}, Default Model={model_default_threshold}, Strong Override Sent={strong_override_sentiment_threshold}, Neg Sent Unhealthy Avoidance Override={negative_sentiment_unhealthy_avoidance_override_threshold}")
        logger.info(f"Model Prob Unhealthy: {probability_unhealthy:.4f}, Prob Healthy: {probability_healthy:.4f}")
        logger.info(f"Sentiment Polarity: {sentiment_polarity:.4f}")
        logger.info(f"Contains Any Unhealthy Kwd: {contains_any_unhealthy_kwd}, Strong Unhealthy Kwd: {contains_strong_unhealthy_kwd}, Any Healthy Kwd: {contains_any_healthy_kwd}, Avoidance Kwd: {contains_avoidance_kwd}, Warning Kwd: {contains_warning_kwd}")
        logger.info("--------------------------------------------")

        final_classification = "healthy" # Default initial classification
        classification_detail = "Model default"

        if contains_any_unhealthy_kwd and contains_warning_kwd and sentiment_polarity < negative_sentiment_threshold:
            final_classification = "healthy"
            classification_detail = "Rule 0: Unhealthy + Warning + Negative Sentiment (Awareness)"
            logger.info("✅ Post-processing Rule 0 Applied: Classified as HEALTHY (Awareness Statement).")

        elif contains_strong_unhealthy_kwd:
            if (sentiment_polarity > strong_override_sentiment_threshold and
                    contains_avoidance_kwd and
                    not contains_any_healthy_kwd):
                final_classification = "healthy"
                classification_detail = "Rule 1 - Override Strong Unhealthy: Strong unhealthy, BUT VERY positive sentiment + Avoidance (no healthy)"
                logger.info("✅ Post-processing override (Rule 1 - Override Strong Unhealthy) Applied: Classified as HEALTHY.")
            else:
                final_classification = "unhealthy"
                classification_detail = "Rule 1: Strong unhealthy keyword"
                logger.info("🚨 Post-processing Rule 1 Applied: Classified as UNHEALTHY.")

        elif contains_any_unhealthy_kwd:
            if (sentiment_polarity < negative_sentiment_unhealthy_avoidance_override_threshold and
                    contains_avoidance_kwd):
                final_classification = "healthy"
                classification_detail = "Rule 2a - Negative Sentiment + Avoidance on Unhealthy"
                logger.info("✅ Post-processing override (Rule 2a - Negative Sentiment + Avoidance on Unhealthy) Applied: Classified as HEALTHY.")
            elif probability_unhealthy > model_high_confidence_threshold:
                final_classification = "unhealthy"
                classification_detail = "Rule 2b: Any unhealthy + High unhealthy probability"
                logger.info("🚨 Post-processing Rule 2b Applied: Classified as UNHEALTHY.")
            elif probability_healthy > model_high_confidence_threshold and sentiment_polarity < negative_sentiment_threshold:
                final_classification = "unhealthy"
                classification_detail = "Rule 2c - Override Any Unhealthy: Any unhealthy + High healthy prob + Negative sentiment"
                logger.info("🚨 Post-processing override (Rule 2c - Override Any Unhealthy) Applied: Classified as UNHEALTHY.")
            else:
                if probability_unhealthy > model_default_threshold:
                    final_classification = "unhealthy"
                    classification_detail = "Rule 2d Default: Any unhealthy, Model UNHEALTHY"
                    logger.info("🚨 Model Classified (Rule 2d Default) Applied: Classified as UNHEALTHY.")
                else:
                    final_classification = "healthy"
                    classification_detail = "Rule 2d Default: Any unhealthy, Model HEALTHY"
                    logger.info("✅ Model Classified (Rule 2d Default) Applied: Classified as HEALTHY.")

        elif contains_any_healthy_kwd:
            if probability_healthy > model_high_confidence_threshold:
                final_classification = "healthy"
                classification_detail = "Rule 3a: Any healthy + High healthy probability"
                logger.info("✅ Post-processing Rule 3a Applied: Classified as HEALTHY.")
            elif probability_unhealthy > model_high_confidence_threshold and sentiment_polarity > positive_sentiment_threshold:
                final_classification = "healthy"
                classification_detail = "Rule 3b - Override Any Healthy: Any healthy + High unhealthy prob + Positive sentiment"
                logger.info("✅ Post-processing override (Rule 3b - Override Any Healthy) Applied: Classified as HEALTHY.")
            else:
                if probability_unhealthy > model_default_threshold:
                    final_classification = "unhealthy"
                    classification_detail = "Rule 3c Default: Any healthy, Model UNHEALTHY"
                    logger.info("🚨 Model Classified (Rule 3c Default) Applied: Classified as UNHEALTHY.")
                else:
                    final_classification = "healthy"
                    classification_detail = "Rule 3c Default: Any healthy, Model HEALTHY"
                    logger.info("✅ Model Classified (Rule 3c Default) Applied: Classified as HEALTHY.")

        else:
            if probability_unhealthy > model_default_threshold:
                final_classification = "unhealthy"
                classification_detail = "Rule 4 - Default: No keyword, Model UNHEALTHY"
                logger.info("🚨 Model Classified (Rule 4 - Default) Applied: Classified as UNHEALTHY.")
            else:
                final_classification = "healthy"
                classification_detail = "Rule 4 - Default: No keyword, Model HEALTHY"
                logger.info("✅ Model Classified (Rule 4 - Default) Applied: Classified as HEALTHY.")

        logger.info(f"--- Final Classification: {final_classification} ---")

    except Exception as e:
        logger.error(f"❌ Error during post-processing logic: {e}")
        final_classification = "Could not finalize classification (Post-processing Error)"
        detail = f"Error during post-processing logic: {e}"[:100] + "..."
        persuasive_message = "No message generated due to processing error."

        fallback_category_from_model = "unhealthy" if model_confidence > model_default_threshold else "healthy"
        final_classification = f"{fallback_category_from_model.capitalize()} (Post-processing Error Fallback)"
        detail = f"Post-processing Error: {e}"[:100] + "... Falling back to model prediction."

        if fallback_category_from_model == "unhealthy":
            persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
        else:
            persuasive_message = "No message generated (classified as healthy by fallback)."
        logger.info(f"Fallback Classification: {final_classification}, Detail: {detail}")
        return {
            "category": final_classification,
            "confidence": model_confidence,
            "detail": detail,
            "persuasive_message": persuasive_message
        }

    persuasive_message = "No message generated."
    if final_classification == "unhealthy":
        persuasive_message = await generate_persuasive_message(original_text) # Await LLM call
    else:
        persuasive_message = "No message generated (classified as healthy)."

    return {
        "category": final_classification.capitalize(),
        "confidence": model_confidence,
        "detail": classification_detail,
        "persuasive_message": persuasive_message
    }

# --- Request Models ---
class PostRequest(BaseModel):
    text: str

class AsyncPostRequest(BaseModel):
    tweet_id: str
    text: str

# --- Synchronous Endpoint (Original) ---
@app.post("/classify_post")
async def classify_sync(post: PostRequest): # Changed to async as _classify_text_logic is async
    """
    Receives text input, preprocesses it, classifies using the Keras model,
    applies post-processing rules based on sentiment and keywords,
    and returns the final classification, model confidence, and potentially
    a persuasive message if classified as unhealthy.
    This is the blocking/synchronous version (relative to the FastAPI server).
    """
    logger.info(f"\n[SYNCHRONOUS] Received Text: '{post.text}'")
    result = await _classify_text_logic(post.text) # Await the shared logic
    logger.info(f"[SYNCHRONOUS] Classification complete.")
    return result

# --- Background Task for Appwrite Update ---
async def _update_appwrite_tweet(
        tweet_id: str,
        category: str,
        persuasive_message: str,
        confidence: float,
        detail: str
):
    logger.info(f"--- Starting Appwrite update for tweet {tweet_id} ---")
    headers = {
        "Content-Type": "application/json",
        "X-Appwrite-Project": APPWRITE_PROJECT_ID,
        "X-Appwrite-Key": APPWRITE_API_KEY,
    }
    update_data = {
        "category": category,
        "persuasiveMessage": persuasive_message,
        "confidence": confidence, # Store confidence and detail too if your model allows
        "detail": detail
    }
    appwrite_url = f"{APPWRITE_ENDPOINT}/databases/{APPWRITE_DATABASE_ID}/collections/{APPWRITE_TWEETS_COLLECTION_ID}/documents/{tweet_id}"

    try:
        async with httpx.AsyncClient() as client:
            response = await client.patch(appwrite_url, headers=headers, json=update_data, timeout=10.0) # Add timeout
        response.raise_for_status() # Raises an exception for 4xx/5xx responses
        logger.info(f"✅ Successfully updated tweet {tweet_id} in Appwrite.")
    except httpx.HTTPStatusError as e:
        logger.error(f"❌ Error updating tweet {tweet_id} in Appwrite (HTTP Status): {e.response.status_code} - {e.response.text}")
    except httpx.RequestError as e:
        logger.error(f"❌ Error updating tweet {tweet_id} in Appwrite (Request Error): {e}")
    except Exception as e:
        logger.error(f"❌ An unexpected error occurred while updating tweet {tweet_id} in Appwrite: {e}")

# --- Asynchronous Endpoint (New) ---
@app.post("/classify_post_async")
async def classify_async(post: AsyncPostRequest, background_tasks: BackgroundTasks):
    """
    Receives tweet text and ID, triggers classification in a background task,
    and immediately returns a confirmation. The Appwrite document will be
    updated later by the background task.
    """
    logger.info(f"\n[ASYNCHRONOUS] Received Text: '{post.text}' for tweet ID: {post.tweet_id}")

    # Add the classification and Appwrite update to background tasks
    # We pass the tweet_id to the background task so it knows which document to update
    async def run_classification_and_update():
        try:
            # Perform the core classification logic
            classification_result = await _classify_text_logic(post.text)

            # Extract results
            category = classification_result.get("category", "Error")
            persuasive_message = classification_result.get("persuasive_message", "Error generating message.")
            confidence = classification_result.get("confidence", 0.0)
            detail = classification_result.get("detail", "No detail.")

            # Update Appwrite
            await _update_appwrite_tweet(post.tweet_id, category, persuasive_message, confidence, detail)
        except Exception as e:
            logger.critical(f"❌ Critical error in background task for tweet {post.tweet_id}: {e}")
            # Optionally, you could log this to an error tracking system
            # or update the tweet in Appwrite with an "error" status.

    background_tasks.add_task(run_classification_and_update)

    logger.info(f"[ASYNCHRONOUS] Classification process started in background for tweet ID: {post.tweet_id}.")
    return {"message": "Classification started in background. Tweet will be updated shortly.", "tweet_id": post.tweet_id}

# Health check endpoint
@app.get("/health")
async def health_check():
    return {"status": "ok"}